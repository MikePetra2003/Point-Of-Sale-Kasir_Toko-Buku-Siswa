...
@section('title', 'Edit Produk Barang - POS SYSTEM')
@section('page-title', 'Edit Produk Barang')
...
<div class="container-fluid px-0" style="max-width: 850px;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3 class="fw-bold text-dark mb-1"><i class="bi bi-pencil-square text-warning me-2"></i>Edit Barang</h3>
            <p class="text-muted small mb-0">Ubah detail data barang inventaris produk Anda.</p>
        </div>
    ...
    <div class="form-card">
        @php
            $satuanTambahan = $barang->barangSatuan
                ->where('is_satuan_dasar', false)
                ->values();
        @endphp
        ...
        <form action="{{ route('barang.update', $barang->id) }}" method="POST">
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    ...
                    <input type="text" name="kode_barang" class="form-control custom-input" placeholder="Contoh: BRC0001" value="{{ old('kode_barang', $barang->kode_barang) }}" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label custom-label">Nama Barang *</label>
                    <input type="text" name="nama_barang" class="form-control custom-input" placeholder="Masukkan nama produk" value="{{ old('nama_barang', $barang->nama_barang) }}" required>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select name="satuan_id" class="form-select custom-select" required>
                        ...
                        @foreach ($satuan as $item)
                            <option value="{{ $item->id }}" {{ old('satuan_id', $barang->satuan_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->nama_satuan }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select name="supplier_id" class="form-select custom-select">
                        ...
                        @foreach ($supplier as $item)
                            <option value="{{ $item->id }}" {{ old('supplier_id', $barang->supplier_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->nama_supplier }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    ...
                    <input type="number" name="harga_beli" class="form-control custom-input" value="{{ old('harga_beli', $barang->harga_beli) }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label custom-label">Harga Jual (Rp) *</label>
                    <input type="number" name="harga_jual" class="form-control custom-input" value="{{ old('harga_jual', $barang->harga_jual) }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label custom-label">Stok Sekarang *</label>
                    <input type="number" name="stok" class="form-control custom-input" value="{{ old('stok', $barang->stok) }}" required>
                </div>
            </div>
            ...
            <p class="text-muted small mb-3">
                Satuan utama menjadi satuan dasar stok. Contoh: jika satuan utama pcs, maka 1 pack bisa diisi 12 pcs.
            </p>

            @for ($i = 0; $i < 3; $i++)
                @php($row = $satuanTambahan[$i] ?? null)
                <div class="row g-3 mb-3">
                    <div class="col-md-3">
                        ...
                        <select name="satuan_tambahan[{{ $i }}][satuan_id]" class="form-select custom-select">
                            ...
                            @foreach ($satuan as $item)
                                <option value="{{ $item->id }}" {{ old("satuan_tambahan.$i.satuan_id", $row?->satuan_id) == $item->id ? 'selected' : '' }}>
                                    {{ $item->nama_satuan }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" name="satuan_tambahan[{{ $i }}][konversi_ke_satuan_dasar]" class="form-control custom-input"
                            value="{{ old("satuan_tambahan.$i.konversi_ke_satuan_dasar", $row?->konversi_ke_satuan_dasar) }}" min="2" placeholder="Contoh: 12">
                    </div>
                    <div class="col-md-3">
                        ...
                        <input type="number" name="satuan_tambahan[{{ $i }}][harga_beli]" class="form-control custom-input"
                            value="{{ old("satuan_tambahan.$i.harga_beli", $row?->harga_beli) }}" min="0" placeholder="Harga beli pack/box">
                    </div>
                    <div class="col-md-3">
                        ...
                        <input type="number" name="satuan_tambahan[{{ $i }}][harga_jual]" class="form-control custom-input"
                            value="{{ old("satuan_tambahan.$i.harga_jual", $row?->harga_jual) }}" min="0" placeholder="Harga jual pack/box">
                    </div>
                </div>
            @endfor
            ...
            <div class="d-flex justify-content-end gap-2">
                ...
                <button type="submit" class="btn btn-save">
                    <i class="bi bi-check-circle-fill me-1"></i> Perbarui Produk
                </button>
            </div>
        </form>
    </div>
